...
export class ImageCropperComponent implements OnChanges {
    ...
    private checkCropperPosition(maintainSize = false): void {
        ...
        console.log(this.maxSize.height);
        console.log(this.maxSize.width);
        
        var h = 1123;
        var w = 794;
        var scale_y = this.maxSize.height / h;
        var scale_x = this.maxSize.width / w;
        
        //76 68 73 20
        //102 456 287 18 Unnamed10
        //301,127,228,23
    ...
    crop(outputType: OutputType = this.outputType): ImageCroppedEvent | Promise<ImageCroppedEvent> | null {
        if (this.sourceImage.nativeElement && this.originalImage != null) {
            ...
            if (ctx) {
                ...
                var resizeRatio =1; //this.getResizeRatio(width);
                if (resizeRatio !== 1) {
                    output.width = Math.round(width * resizeRatio);
                    output.height = this.maintainAspectRatio
                        ? Math.round(output.width / this.aspectRatio)
                        : Math.round(height * resizeRatio);
                    resizeCanvas(cropCanvas, output.width*2, output.height*2);
                }
